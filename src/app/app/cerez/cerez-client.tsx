...
function CerezForm({ clientId }: { clientId: string }) {
  ...
  async function onGenerate() {
    ...
    try {
      await generateCerezStream(clientId, { site, tools, cmp, kategoriler }, {
        onGrounding: (g) => {
          grounding = g;
          flush(true);
        },
  ...
  async function onDownload() {
    ...
    setDownloading(true);
    try {
      const blob = await cerezDocx(clientId, result.text, "Çerez Politikası");
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "cerez-politikasi.docx";
      a.click();
